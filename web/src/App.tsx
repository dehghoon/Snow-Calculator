...
const help: Record<HelpKey, { title: string; reference: string; visual: ReactNode; body: ReactNode }> = {
  slope: {
    ...
    body: <><p>Enter the actual roof angle measured from horizontal.</p><p className="help-note">The engine calculates the applicable slope factor internally from the roof slope and surface condition.</p></>,
  },
  surface: {
    ...
    body: <><p><b>Normal</b> — ordinary roof surface condition.</p><p><b>Smooth / slippery</b> — use only where the roof is unobstructed and snow and ice can slide completely off the roof.</p><p className="help-note">If parapets, obstructions or roof geometry prevent complete sliding, do not assume the slippery-roof condition.</p></>,
  },
  is: {
  ...
  l: {
    title: "Larger plan dimension, l", reference: "NBCC 2020 4.1.6.2.(2)",
    visual: <svg viewBox="0 0 220 90"><rect x="25" y="22" width="170" height="48"/><path d="M35 79H185"/><text x="104" y="87">l</text></svg>,
    body: <><p><b>l</b> is the larger plan dimension of the roof, in metres.</p><p>Use the applicable upper- or lower-roof plan dimension for the roof whose basic roof factor is being determined.</p><p className="help-note">For the NBCC characteristic-length expression, l should be the larger plan dimension and must be greater than zero.</p></>,
  },
  w: {
    title: "Smaller plan dimension, w", reference: "NBCC 2020 4.1.6.2.(2)",
    visual: <svg viewBox="0 0 220 90"><rect x="42" y="15" width="136" height="60"/><path d="M31 21V69"/><text x="14" y="49">w</text></svg>,
    body: <><p><b>w</b> is the smaller plan dimension of the roof, in metres.</p><p>Use the corresponding smaller dimension of the applicable roof plan.</p><p className="help-note">The intended geometry is w ≤ l.</p></>,
  },
  lc: {
    title: "Characteristic roof length, lc", reference: "NBCC 2020 4.1.6.2.(2)",
    visual: <svg viewBox="0 0 220 90"><rect x="32" y="18" width="156" height="54"/><text x="72" y="48">lc = 2w − w²/l</text></svg>,
    body: <><div className="help-callout"><b>Calculated automatically</b><span>lc = 2w − w²/l</span></div><p>The app calculates lc from the entered roof plan dimensions and uses it to calculate Cb.</p><p className="help-note">lc is not a user-entered factor.</p></>,
  },
  cb: {
    title: "Calculated basic roof factor, Cb", reference: "NBCC 2020 4.1.6.2.(2)",
    visual: <svg viewBox="0 0 220 90"><path d="M30 65h160M50 65l55-38 65 38"/><text x="89" y="50">Cb</text></svg>,
    body: <><div className="help-callout"><b>Calculated automatically</b><span>Cb = 0.80 when lc ≤ 70/Cw².</span></div><p>When <b>lc &gt; 70/Cw²</b>, the app uses the NBCC expression:</p><p><b>Cb = (1/Cw)[1 − (1 − 0.8Cw) exp(−(lcCw² − 70)/100)]</b></p><p className="help-note">NBCC also contains a separate low-mean-roof-height condition where Cb = 1.0. This calculator currently applies the plan-geometry branch shown above.</p></>,
  },
  case: {
    ...
    body: <><table className="help-table"><tbody><tr><td><b>Case I</b></td><td>Upper-roof snow source contributing to the lower receiving roof; β = 1.00.</td></tr><tr><td><b>Case II</b></td><td>Applicable lower-roof source toward the roof step; β = 0.67.</td></tr><tr><td><b>Case III</b></td><td>Alternate lower-roof source configuration/direction where geometry permits; β = 0.67.</td></tr></tbody></table><p className="help-note">Each case has its own source area and its own ls / ws mapping.</p></>,
  },
  ls: {
    ...
    body: <><p><b>ls</b> is the physical source-area length for the selected drift case.</p><p className="help-note">Use verified project geometry; do not scale it from a Code figure.</p></>,
  },
  ws: {
    ...
    body: <><p><b>ws</b> is the physical source-area width for the selected drift case.</p><p className="help-note">Each applicable case can have a different ws.</p></>,
  },
  h: {
    ...
    body: <><p><b>h</b> is the vertical roof-level difference at the step that creates the sheltered lower-roof drift condition.</p><p className="help-note">Enter the actual project roof-level difference.</p></>,
  },
  hp: {
    title: "Parapet height, hp", reference: "NBCC Figure 4.1.6.5.-A / Commentary geometry",
    visual: <svg viewBox="0 0 220 90"><path d="M25 58H180M55 58V27"/><path d="M70 27V58"/><text x="77" y="45">hp</text></svg>,
    body: <><p><b>hp</b> is the vertical parapet height above the applicable roof surface.</p><p className="help-note">Use the actual project parapet location and height; the reference figure is only a geometry guide.</p></>,
  },
};
...
export default function App() {
  ...
  const cb = cw > 0 && Number.isFinite(lc)
    ? (lc <= cbThreshold ? 0.8 : (1 / cw) * (1 - (1 - 0.8 * cw) * Math.exp(-(lcCw2 - 70) / 100)))
    : 0.8;
  ...
  async function submit(e: FormEvent) { e.preventDefault(); setBusy(true); setError(""); if (!(roofL > 0) || !(roofW >= 0) || roofW > roofL || !(cw > 0)) { setBusy(false); setResult(null); setError("Check Cb geometry inputs: use l > 0, 0 ≤ w ≤ l, and Cw > 0."); return; } try { setResult(await calculateRoofSnow(payload)); } catch (err) { setResult(null); setError(err instanceof Error ? err.message : "Calculation failed."); } finally { setBusy(false); } }
