...
function ReferenceFigure({ src, title, reference }: { src: string; title: string; reference: string }) {
  return <aside className="engineering-figure" aria-label={title}>
    <div className="figure-heading"><div><span>GEOMETRY REFERENCE</span><b>{title}</b></div><small>{reference}</small></div>
    <img src={src} alt={title}/>
    <p>Use this diagram to identify the physical dimensions only. Enter verified project dimensions; do not scale values from the figure.</p>
  </aside>;
}
...
export default function App() {
  ...
  const [provinces, setProvinces] = useState<string[]>([]), [locations, setLocations] = useState<string[]>([]);
  ...
  async function changeProvince(next: string) {
    setProvince(next); setLocation(""); setLocations([]); setClimaticSource(""); setResult(null);
    if (!next) return;
    try { setError(""); setLocations(await getClimaticLocations(next)); } catch (err) { setError(err instanceof Error ? err.message : "Failed to load locations."); }
  }
  async function changeLocation(next: string) {
    setLocation(next); setResult(null); if (!province || !next) return;
    try { setError(""); const data = await getClimaticLocation(province, next); setSs(data.ss); setSr(data.sr); setClimaticSource(data.source); }
    catch (err) { setError(err instanceof Error ? err.message : "Failed to load climatic data."); }
  }
  ...
  return <main>
    ...
    <div className="layout">
      <form className="panel" onSubmit={submit}>
        ...
        <div className="grid"><NumberField label="Ground snow load, Ss" unit="kPa" value={ss} onChange={setSs} readOnly={Boolean(location)}/><NumberField label="Associated rain load, Sr" unit="kPa" value={sr} onChange={setSr} readOnly={Boolean(location)}/><NumberField label="Roof slope, α" unit="deg" value={slope} onChange={setSlope}/><label className="field"><span>Roof surface</span><select value={surface} onChange={e => setSurface(e.target.value as typeof surface)}><option value="normal">Normal</option><option value="smooth_slippery">Smooth / slippery</option></select></label><NumberField label="Importance factor, Is" value={isFactor} onChange={setIsFactor}/><NumberField label="Wind exposure factor, Cw" value={cw} onChange={setCw}/><NumberField label="Basic roof factor, Cb" value={cb} onChange={setCb}/></div>

        {mode === "LOWER_ADJACENT_ROOF" && <div className="input-figure-layout"><section className="mode geometry-inputs"><p><strong>Source-area geometry</strong></p><p className="note">The figure beside these inputs identifies the corresponding physical dimensions.</p><label className="field"><span>Case</span><select value={caseId} onChange={e => setCaseId(e.target.value as typeof caseId)}><option>I</option><option>II</option><option>III</option></select></label><div className="grid"><NumberField label="Source length, ls" unit="m" value={ls} onChange={setLs}/><NumberField label="Source width, ws" unit="m" value={ws} onChange={setWs}/><NumberField label="Roof step height, h" unit="m" value={stepHeight} onChange={setStepHeight}/><NumberField label="Parapet height, hp" unit="m" value={parapetHeight} onChange={setParapetHeight}/></div>{cw !== 1 && <div className="error">Adjacent-surface drift requires Cw = 1.0.</div>}</section><ReferenceFigure src="/figures/lower-adjacent-roof.svg" title="Lower adjacent roof / drift geometry" reference="Workbook Figure G-5"/></div>}
