(() => {
  function showImage(pop, src, alt) {
    ...
    if (visual) {
      ...
      visual.innerHTML = `<img src="${src}" alt="${alt}" style="display:block;width:100%;height:auto;max-height:none;object-fit:contain;background:#fff;border-radius:8px"/>`;
    }
  ...
  function enhance() {
    document.querySelectorAll('.help-popover').forEach(pop => {
      ...
      if (label === 'Lower-roof source case') {
        showImage(pop, '/nbcc-roof-drop-cases.webp?v=2', 'NBCC Figure 4.1.6.5.-B - Snow load Cases I, II and III');
