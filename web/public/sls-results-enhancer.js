(() => {
  ...
  function enhanceReport(data) {
    ...
    // Remove the internal Formula ID column from the user-facing NBCC references table.
    [...report.querySelectorAll('section')].forEach(section => {
      ...
      if (head) {
        ...
        if (ths[0]) ths[0].textContent = 'NBCC 2020 Reference';
      }
    ...
    if (resultBody && !resultBody.querySelector('[data-limit-state="SLS"]')) {
      if (final.sls_peak_snow_load_kpa != null) {
    ...
    if (distTable && uls.length && sls.length && !distTable.dataset.slsEnhanced) {
